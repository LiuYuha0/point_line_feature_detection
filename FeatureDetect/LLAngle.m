function [angles, modgrad, list] = LLAngle(scaled_image, threshold, n_bins, dense_img)
% 
% Finds the angles and the gradients of the image. Generates a list of pseudo ordered points. 
% 
% input:
%      scaled_image the image after guassian blur and resize
%      threshold    The minimum value of the angle that is considered defined, otherwise NOTDEF 
%      n_bins       The number of bins with which gradients are ordered by, using bucket sort. 
% 
% output:
%      list = [y, x, gradient]
%                   Vector of coordinate points that are pseudo ordered by magnitude. 
%      *            Pixels would be ordered by norm value, up to a precision given by max_grad/n_bins. 
% list ordered by gradient
