...
for i = 1: size(reg, 1)
    ...
    if distance([xc, yc], [reg(i,2), reg(i,1)]) < rec(5)
        angle = reg(i, 4);
        ang_d = AngleDiffSigned(angle, ang_c);
        sum = sum + ang_d;
        s_sum = s_sum + ang_d^2;
        n = n + 1;  
    end
...
%Try new region
[reg, reg_angle, updated_used_] = RegionGrow([reg(1, 1), reg(1, 2), 0], used, angles, modgrad, tau);
...
if density < density_th
    [y, updated_used, rec_] = ReduceRegionRadius(reg, reg_angle, prec, p, rec, density, density_th, used);
else
    y = true;
    return;
end
